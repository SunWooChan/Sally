...
# 서버 DB qnas table에서 가져온 것

survey = pd.read_excel('survey.xlsx',engine='openpyxl', index_col=0) # user id to survey

vege2survey = pd.read_csv('vege2survey_remove.csv',index_col=0) # vege to survey
...
dict_survey = dict(survey.iloc[-1]) # 맨 마지막 user 선택
...
print(sorted_dict[:3]) # setting want to print num
